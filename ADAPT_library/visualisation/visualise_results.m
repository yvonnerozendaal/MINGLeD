    ...
    if isfield(R(1).spline_data,item_name)        
        for i_it = 1:m.Niter
            ...
            plot(m.plotinfo.ax(ind_item),t_data,m_data,'o',...
                    'Color',clist(i_it,:),'LineWidth',m.plotinfo.LW_spline/2,'MarkerSize',m.plotinfo.MS_data/2)
        end  
    else
        h = 0;
    end
...
function annotate_graph(R,m,h,ind_item)
    ...
    % -- axes information
    xlabel(m.plotinfo.ax(ind_item),['time [' m.info.t_unit ']'])
    ylabel(m.plotinfo.ax(ind_item),['[' unit ']'])
    if m.plotinfo.save
%         title(m.plotinfo.ax(ind_item),[full_name ' (' short_name ')'])
        title(m.plotinfo.ax(ind_item),full_name)
    else
%         title(m.plotinfo.ax(ind_item),[full_name ' (' short_name ')'],'FontWeight','bold')
        title(m.plotinfo.ax(ind_item),full_name,'FontWeight','bold')
    end
%     title(m.plotinfo.ax(ind_item),[short_name ': ' full_name])
    set(m.plotinfo.ax(ind_item),'Xlim',[-0.05 1.05]*m.info.t_max)
        ...
        switch m.plotinfo.type
            ...
            otherwise
                if isfield(m.plotinfo.limit,'x')
                    set(m.plotinfo.ax(ind_item),'Xlim',m.plotinfo.limit.x(ind_item,:));
                end
        end
    else
%         ylim_curr = get(m.plotinfo.ax(ind_item),'Ylim');
%         if ylim_curr(1)<0
%             ylim_curr(1) = 0;
%         end
%         set(m.plotinfo.ax(ind_item),'Ylim',ylim_curr);        
    end
